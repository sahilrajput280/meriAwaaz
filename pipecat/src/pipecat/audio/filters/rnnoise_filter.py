...
class RNNoiseFilter(BaseAudioFilter):
    ...
    The ``speech_prob_threshold`` parameter controls noise ambiguity handling:
    frames whose RNNoise speech probability falls below the threshold are replaced
    with silence, preventing ambiguous background noise from reaching VAD/STT.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            speech_prob_threshold: RNNoise speech probability threshold in [0.0, 1.0].
                                   Frames whose speech probability is below this value
                                   are replaced with silence. A value of 0.0 (default)
                                   disables threshold gating so all denoised audio is
                                   passed through. Recommended starting value: 0.5.
        """
        ...
        self._speech_prob_threshold = speech_prob_threshold
    ...
    async def filter(self, audio: bytes) -> bytes:
        ...
        for speech_prob, denoised_frame in self._rnnoise.denoise_chunk(audio_samples):
            ...
            # Noise ambiguity gating: replace frames below threshold with silence.
            # This prevents ambiguous background noise from reaching VAD/STT.
            if self._speech_prob_threshold > 0.0 and speech_prob < self._speech_prob_threshold:
                denoised_int16 = np.zeros_like(denoised_int16)
